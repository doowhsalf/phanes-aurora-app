  ...
  try {
    ...
    async function traverseNode(node, parentId = null) {
      ...
      try {
        ...
        const newNode = await nodeCollection.insertOne(nodeWithoutChildren);
        console.log(`Inserted node: ${newNode.insertedId}`);

        if (parentId) {
          const newLink = await nodeLinksCollection.insertOne({
            parentId,
            childId: newNode.insertedId, // Changed from node._id to use _id of the inserted node
            linkToContext: nodeWithoutChildren.linkToContext.toLowerCase(),
            linkFromContext: nodeWithoutChildren.linkFromContext.toLowerCase(),
          });
          console.log(`Inserted link: ${newLink.insertedId}`);
        }

        if (Array.isArray(children)) {
          for (const child of children) {
            await traverseNode(child, nodeWithoutChildren._id); // Changed to pass the new _id
          }
        }
      } catch (err) {
