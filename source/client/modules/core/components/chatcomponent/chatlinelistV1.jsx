...
class ChatLineList extends React.Component {
  ...
  renderLine2(line, menuComponent, firstLine, lastLine, autoscroll) {
    ...
    const lineTextClassName = myLine
      ? classes.chatLinePerson +
        " " +
      ...
      : classes.chatLinePerson +
        ...
        theirGroup +
        " " +
        (themeModeDark === true ? classes.fromThemDark : classes.fromThemLight);
    ...
    let stuff = (
      ...
      >
        {firstLine && lastLine ? (
          <div>
            {myLine ? null : (
              <div className={classes.otherUserName}> {line.createdByName}</div>
            )}
            <React.Fragment>
              {myLine ? null : (
              ...
              <div className={AdjustClass}>
                <React.Fragment>
                  <span key={line._id + "user"} className={lineTextClassName}>
                    <Typography variant="body2" className={classes.inline}>
                      {this._renderText(line.text)}
                    </Typography>
                  </span>
                </React.Fragment>
              </div>
          ...
          <div>
            {myLine ? null : lastLine ? (
              <div className={classes.chatAvatar}>{avatar}</div>
            ) : null}
            {myLine ? null : firstLine ? (
              <div className={classes.otherUserName}> {line.createdByName}</div>
            ) : null}
            <React.Fragment>
              <span key={line._id + "user"} className={lineTextClassName}>
  ...
  _linkify(text) {
    ...
    return text.replace(urlRegex, function (url) {
      ...
      if (url !== undefined) {
        let isAnImage = url.match(/\.(jpeg|jpg|gif|png)$/) != null;
        if (isAnImage) {
          let newMessage =
            text +
            "\n\n" +
            '<img src="' +
            url +
            '" width="300" height="auto"</img>';
          return newMessage;
          //return '<img src="' + url + '" width="300" height="auto"</img>';
        } else {
