...
*/
DEFCON5 && console.log("In workOrdersCustomerOrder");

export const composer = ({ context, contentId }, onData) => {
  const { Meteor, Collections } = context();
  DEFCON5 && console.log("In nodes component composer");
  DEFCON5 && console.log(contentId);

  // this.filter = (articles, dataSelector) => {
  //   if (articles) {
  ...
  if (Meteor.subscribe("workorders.open.search", dataSelector, 100).ready()) {
    // var projection = {
    //   _id: "$_id",
    ...
    DEFCON5 && console.log("workorders.open.search - Get All Node");
    /*      */
