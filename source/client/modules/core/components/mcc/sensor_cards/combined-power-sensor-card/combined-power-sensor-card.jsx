...
function SensorCard({ sensorData, historyData }) {
  ...
  const [prevPower, setPrevPower] = useState(null);
  const previousPowerRef = useRef(sensorData.Power);
  const displayedData =
    historyData && historyData[timePeriod] ? historyData[timePeriod] : {};

  useEffect(() => {
    // Only update the previous Power when the current Power changes
    if (sensorData.Power !== prevPower) {
      setPrevPower(sensorData.Power);
    }
  }, [sensorData.Power]);

  useEffect(() => {
    previousPowerRef.current = prevPower;
  }, [prevPower]);
  ...
  // if (previousPowerRef.current !== null) {
  //   if (sensorData.Power > previousPowerRef.current) {
  //     sensorChangeStyle.color = theme.palette.primary.light; // For increased Power
  //   } else if (sensorData.Power < previousPowerRef.current) {
  //     sensorChangeStyle.color = theme.palette.primary.dark; // For decreased Power
  //   } else {
  //     sensorChangeStyle.color = theme.palette.primary.main; // No change in Power
  //   }
  // }
 ...
 */
      ...
      >
        ...
        >
          <Typography
            ...
            key={Date.now()}
          >
            {sensorData.voltage} voltage (any)
          </Typography>
          <Typography
            variant="body2"
            style={sensorChangeStyleSecondary}
            key={Date.now()}
          >
            {sensorData.voltage_L1} voltage (L1)
          </Typography>
          <Typography
            variant="body2"
            style={sensorChangeStyleSecondary}
            key={Date.now()}
          >
            {sensorData.voltage_L2} voltage (L2)
          </Typography>
          <Typography
            variant="body2"
            style={sensorChangeStyleSecondary}
            key={Date.now()}
          >
            {sensorData.voltage_L3} voltage (L3)
          </Typography>
  ...
  DEFCON2 && console.log("sensorData - Combined", sensorData);
