import SensorCard from "./combined-power-sensor-card.jsx";
...
import { sensorDataUtil } from "../sensorDataUtil.jsx";
  ...
  if (Meteor.subscribe("nodes.getTree", theNodeId).ready()) {
    ...
    Meteor.call("getTreeStructure", theNodeId, (err, result) => {
      if (err) {
        ...
        var tree = result;
        let sensorData = sensorDataUtil(tree, "active_power_sensor");
        ...
        Meteor.call(
          ...
          (err, result) => {
            if (err) {
              ...
              DEFCON2 &&
                console.log("historyDataSimulated Data:", historyDataSimulated);
