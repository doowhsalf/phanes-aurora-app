...
const styles = (theme) => ({
  ...
  row_inactive_role_light: {
    backgroundColor: "#d1c4e9",
    color: theme.palette.common.white,
  },
  row_inactive_role_dark: {
...
function getRowClassNameByStatus(prio, classes) {
  ...
  switch (prio) {
    case "0":
      rowClassname = themeModeDark
        ? classNames({
            [classes.row_inactive_role_light]: true,
          })
        : classNames({
            [classes.row_inactive_role_dark]: true,
          });
      break;
    case "1":
    default:
    ...
    return (
      <Card className={classes.card}>
        ...
        <Table className={classes.table} size="small">
          ...
          <TableBody>
            {stableSort(rows, getSorting("desc", "fromDateString")).map(
              (row) => (
                <TableRow
