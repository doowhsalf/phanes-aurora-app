import NodesTableData from "../components/mcc/sensormapping_table";
...
DEFCON5 && console.log("In sensormapping component, kicknodesing stuff");
/* 
...
export const composer = ({ context, searchText }, onData) => {
  ...
  DEFCON5 && console.log("In nodes sensormapping composer");
  ...
  if (Meteor.subscribe("sensormapping.all").ready()) {
    const Selector = {};

    const nodes = Collections.SensorMapping.find(Selector).fetch();
